...
import static com.zekry.view.AlarmLandingPageActivity.launchIntent;
...
public class AlarmReceiver  extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        builder.setSmallIcon(R.drawable.ic_access_alarm_white_24dp);
        builder.setColor(ContextCompat.getColor(context, R.color.colorAccent));
        builder.setContentTitle(context.getString(R.string.app_name));
        builder.setContentText("تذكير باذكارك اليوميه");
        builder.setTicker("احفظ الله يحفظك");
    ...
    private static PendingIntent launchAlarmLandingPage(Context ctx, Alarm alarm) {
        saveAlarmId(ctx, alarm.getId());
        return PendingIntent.getActivity(
                ctx, alarm.notificationId(), launchIntent(ctx), FLAG_UPDATE_CURRENT
        );
    }

    private static void saveAlarmId(Context context, long id) {
        SharedPreferences.Editor editor = context.getSharedPreferences("alarm_id", MODE_PRIVATE).edit();
